...
Neste exemplo \ref{exemplo-formatos-de-cores}, cada seletor CSS usa um formato de cor diferente. O primeiro exemplo usa uma cor nomeada \var{red} para o texto de um cabeçalho (\var{h1}). O segundo exemplo usa a sintaxe RGB para definir a cor do texto de um parágrafo (\var{p}) como vermelho. O terceiro exemplo usa a sintaxe RGBA para definir um fundo verde com 50\% de transparência para uma \var{div}. O quarto exemplo usa a sintaxe HSL para definir um plano de fundo com uma matiz verde-azulada para um \var{span}. O quinto exemplo usa a sintaxe HSLA para definir um plano de fundo com uma matiz azul-esverdeada para um link (\var{a}) com 30\% de transparência. Por fim, o sexto exemplo usa a sintaxe hexadecimal para definir a cor do texto de um item de lista (\var{li}) como azul.
...
Neste código \ref{exemplo-gradiente}, o primeiro exemplo usa um gradiente linear para o plano de fundo seletor CSS para a classe \var{header} com uma transição suave entre as cores \#f8f8f8 e \#ffffff. O segundo exemplo usa um gradiente radial para o plano de fundo de elementos do tipo \var{button} com uma transição suave entre as cores \#ffffff e \#008000.
...
\begin{csscode}{Exemplo de uso das unidades \var{em} e \var{rem}.}{exemplo_em_rem}
.exemplo {
...
\begin{csscode}{Exemplo de uso das unidades \var{vw} e \var{vh}.}{exemplo_vw_vh}
.exemplo {
...
\subsection{Propriedade \var{background-color}}

A propriedade \var{background-color} define a cor de fundo de um elemento. Podemos definir a cor usando um dos formatos de cores apresentados anteriormente neste capítulo. O código \ref{bg-color} mostra um exemplo de como definir a cor de fundo de um elemento.

\begin{csscode}{Definindo a cor de fundo com \var{background-color}.}{bg-color}
.exemplo {
...
\subsection{Propriedade \var{background-repeat}}

A propriedade \var{background-repeat} define como a imagem de fundo deve ser repetida dentro do elemento. Existem quatro valores possíveis para essa propriedade: \var{repeat} (repete horizontalmente e verticalmente), \var{repeat-x} (repete apenas horizontalmente), \var{repeat-y} (repete apenas verticalmente) e \var{no-repeat} (não repete). O código \ref{bg-repeat} mostra um exemplo de como definir a repetição da imagem de fundo para um elemento.

\begin{csscode}{Definindo a repetição da imagem de fundo com \var{background-repeat}.}{bg-repeat}
.exemplo {
    background-image: url("imagem.jpg");
...
\subsection{Propriedade \var{background-position}}

A propriedade \var{background-position} define a posição inicial da imagem de fundo dentro do elemento. Podemos definir a posição usando palavras-chave (\var{top}, \var{bottom}, \var{left}, \var{right}, \var{center}) ou valores em porcentagem ou pixels. O código \ref{bg-position} mostra um exemplo de como definir a posição da imagem de fundo para um elemento.

\begin{csscode}{Definindo a posição da imagem de fundo com \var{background-position}.}{bg-position}
.exemplo {
    background-image: url("imagem.jpg");
...
\subsection{Propriedade \var{background-attachment}}

A propriedade \var{background-attachment} define se a imagem de fundo deve se mover com o restante do conteúdo da página ou ficar fixa no lugar enquanto o conteúdo se move. Existem dois valores possíveis para essa propriedade: \var{scroll} (a imagem se move com o restante do conteúdo) e \var{fixed} (a imagem fica fixa no lugar enquanto o conteúdo se move). O código \ref{bg-attachment} mostra um exemplo de como definir a fixação da imagem de fundo para um elemento.

\begin{csscode}{Definindo a fixação da imagem de fundo com \var{background-attachment}.}{bg-attachment}
.exemplo {
    background-image: url("imagem.jpg");
...
\subsection{Propriedade \var{background-size}}

A propriedade \var{background-size} define o tamanho da imagem de fundo em relação ao elemento. Podemos definir o tamanho usando valores em porcentagem, pixels, palavras-chave (\var{cover} e \var{contain}) ou uma combinação de valores para largura e altura. O código \ref{bg-size} mostra um exemplo de como definir o tamanho da imagem de fundo para um elemento.
...
As bordas são importantes para definir o leiaute e o estilo de um elemento HTML. Em CSS, existem diversas propriedades que permitem personalizar a aparência das bordas, como \var{border-color}, \var{border-width}, \var{border-style}, \var{border-radius}, e as propriedades de bordas individuais, como \var{border-left} e \var{border-right}.

\subsection{Propriedade \var{border-color}}

A propriedade \var{border-color} define a cor da borda de um elemento. Ela pode ser definida de várias formas, como por um nome de cor, um valor hexadecimal ou RGB. O código \ref{border-color-exemplo} mostra um exemplo de como definir a cor da borda para um elemento.
...
Neste exemplo \ref{border-color-exemplo}, a borda de um parágrafo é definida com a cor vermelha. É possível também definir a cor das quatro bordas individualmente, usando as propriedades \var{border-top-color}, \var{border-right-color}, \var{border-bottom-color} e \var{border-left-color}.

\subsection{Propriedade \var{border-width}}

A propriedade \var{border-width} define a largura da borda de um elemento. Ela pode ser definida em valores absolutos, como em pixels, ou em valores relativos, como em porcentagem. O código \ref{border-width-exemplo} mostra um exemplo de como definir a largura da borda para um elemento.

\begin{csscode}{Exemplo de uso da propriedade \var{border-width}.}{border-width-exemplo}
p {
    border-width: 2px;
}
\end{csscode}

Neste exemplo \ref{border-width-exemplo}, a borda de um parágrafo é definida com a largura de 2 pixels. É possível também definir a largura das quatro bordas individualmente, usando as propriedades \var{border-top-width}, \var{border-right-width}, \var{border-bottom-width} e \var{border-left-width}.

\subsection{Propriedade \var{border-style}}

A propriedade \var{border-style} define o estilo da borda de um elemento. Existem diversos estilos de bordas, como \var{solid}, \var{dashed}, \var{dotted}, \var{double}, \var{groove}, \var{ridge}, \var{inset} e \var{outset}. O código \ref{border-style-exemplo} mostra um exemplo de como definir o estilo da borda para um elemento.

\begin{csscode}{Exemplo de uso da propriedade \var{border-style}.}{border-style-exemplo}
p {
    border-style: dotted;
}
\end{csscode}

Neste exemplo \ref{border-style-exemplo}, a borda de um parágrafo é definida com o estilo pontilhado. É possível também definir o estilo das quatro bordas individualmente, usando as propriedades \var{border-top-style}, \var{border-right-style}, \var{border-bottom-style} e \var{border-left-style}.

\subsection{Propriedade \var{border-radius}}

A propriedade \var{border-radius} define o raio dos cantos de uma borda. Ela pode ser definida em valores absolutos, como em pixels, ou em valores relativos, como em porcentagem. O código \ref{border-radius-exemplo} mostra um exemplo de como definir o raio dos cantos da borda para um elemento.

\begin{csscode}{Exemplo de uso da propriedade \var{border-radius}.}{border-radius-exemplo}
p {
    border-radius: 10px;
}
\end{csscode}

Neste exemplo \ref{border-radius-exemplo}, os cantos da borda de um parágrafo são definidos com um raio de 10 pixels. É possível também definir o raio dos cantos individualmente, usando as propriedades \var{border-top-left-radius}, \var{border-top-right-radius}, \var{border-bottom-left-radius} e \var{border-bottom-right-radius}.

\subsection{Propriedades Multivaloradas}

Além das propriedades de bordas que aceitam valores individuais, é possível usar a propriedade \var{border} para definir todas as propriedades de borda de um elemento de uma só vez. O código \ref{border-exemplo} mostra um exemplo de como definir a borda para um elemento.

\begin{csscode}{Exemplo de uso da propriedade \var{border}.}{border-exemplo}
p {
    border: 2px dashed red;
}
\end{csscode}

Neste exemplo \ref{border-exemplo}, a borda de um parágrafo é definida com a largura de 2 pixels, o estilo tracejado e a cor vermelha, tudo em uma única propriedade \var{border}. Também é possível usar a propriedade \var{border-left} para definir todas as propriedades de borda da borda esquerda de um elemento de uma só vez. O código \ref{border-left-exemplo} mostra um exemplo de como definir a borda esquerda para um elemento.

\begin{csscode}{Exemplo de uso da propriedade \var{border-left}.}{border-left-exemplo}
p {
    border-left: 2px dashed red;
}
\end{csscode}

Neste exemplo \ref{border-left-exemplo}, a borda esquerda de um parágrafo é definida com a largura de 2 pixels, o estilo tracejado e a cor vermelha, tudo em uma única propriedade \var{border-left}. O uso dessas propriedades pode tornar o código CSS mais simples e legível, especialmente quando se deseja aplicar as mesmas propriedades para todas as bordas ou apenas para uma borda específica de um elemento.

\subsection{Bordas do Tipo \textit{Outline}}

Além das bordas normais, que são definidas pelas propriedades \var{border}, é possível criar bordas do tipo \textit{outline} em CSS. As bordas do tipo \textit{outline} são semelhantes às bordas normais, mas não afetam o tamanho ou o leiaute do elemento HTML e podem ter um afastamento. As bordas do tipo \textit{outline} são definidas pela propriedade \var{outline}, que pode receber valores como largura, estilo e cor. O código \ref{outline-exemplo} mostra um exemplo de como definir a borda do tipo \textit{outline} para um elemento.

\begin{csscode}{Exemplo de uso da propriedade \var{outline}.}{outline-exemplo}
p {
...
As propriedades de configuração de margens são \var{margin} (externa) e \var{padding} (externa). Elas são essenciais para definir a distância entre os elementos HTML em uma página web. É importante entender a diferença entre elas e os possíveis valores que podem ser usados.

A propriedade \var{margin} define a margem em torno do elemento. Os valores podem ser definidos para as margens superior, inferior, esquerda e direita usando as propriedades \var{margin-top}, \var{margin-bottom}, \var{margin-left} e \var{margin-right}, respectivamente. 

Os valores da margem podem ser especificados em \var{px} (pixels), \var{em} (relativos à fonte), \var{rem} (relativos ao tamanho da fonte do elemento pai), \var{\%} (porcentagem) ou \var{auto} (para centralizar o elemento na tela). Por exemplo, o código \ref{exemploMargin} mostra como definir uma margem de 10 pixels em torno de um elemento.

\begin{csscode}{Exemplo de uso da propriedade \var{margin}}{exemploMargin}
div {
    margin: 10px;
}
\end{csscode}

Neste exemplo \ref{exemploMargin}, a propriedade \var{margin} é definida com um valor de 10 pixels, o que significa que a margem em torno do elemento \var{div} será de 10 pixels em todas as direções.

A propriedade \var{padding}, por sua vez, define a margem interna do elemento. Os valores podem ser definidos para a margem superior, inferior, esquerda e direita usando as propriedades \var{padding-top}, \var{padding-bottom}, \var{padding-left} e \var{padding-right}, respectivamente. Os valores podem ser especificados da mesma forma que a propriedade \var{margin}. Por exemplo, o código \ref{exemploPadding} mostra como definir uma margem interna de 10 pixels dentro de um elemento.

\begin{csscode}{Exemplo de margem interna com propriedade \var{padding}.}{exemploPadding}
div {
    padding: 10px;
}
\end{csscode}

Neste exemplo \ref{exemploPadding}, a propriedade \var{padding} é definida com um valor de 10 pixels, o que significa que a margem interna do elemento \var{div} será de 10 pixels em todas as direções. É importante notar que as margens interna e externa podem afetar o leiaute da página, especialmente quando usados em conjunto com outros elementos HTML. 

Em resumo, as propriedades \var{margin} e \var{padding} são importantes para definir a distância entre os elementos HTML em uma página web. Os valores podem ser definidos usando uma das unidades de medida ou o valor \var{auto}, dependendo da necessidade. Ao usar essas propriedades, é importante considerar como elas afetam o leiaute da página e usá-las com moderação.

\subsection{Valor Automático da Propriedade \var{margin}}

O valor \var{auto} da propriedade \var{margin} é usado para centralizar um elemento na tela. Quando a margem é definida como \var{auto}, o navegador define automaticamente um valor para a margem, a fim de centralizar o elemento na tela. Por exemplo, o código \ref{exemploMarginAuto} mostra como centralizar um elemento \var{div} na tela usando a propriedade \var{margin} com o valor \var{auto}.

\begin{csscode}{Exemplo de \var{margin} com valor \var{auto}.}{exemploMarginAuto}
div {
    margin: auto;
    width: 50%;
...
Neste exemplo \ref{exemploMarginAuto}, a propriedade \var{margin} é definida como \var{auto}, o que significa que o navegador definirá automaticamente o valor da margem para centralizar \textbf{horizontalmente} o elemento \var{div} na tela. Além disso, a largura do elemento é definida como 50\%, a borda é definida como 1 pixel, estilo sólido e cor preta. A margem interna é definida como 10 pixels.

É importante notar que o valor \var{auto} da propriedade \var{margin} só funciona em elementos com uma largura definida, como um elemento com a propriedade \var{width} definida. Caso contrário, o navegador não terá como calcular a margem necessária para centralizar o elemento na tela.

Em resumo, o valor \var{auto} da propriedade \var{margin} é usado para centralizar um elemento na tela. É importante definir uma largura para o elemento para que o navegador possa calcular a margem necessária para centralizá-lo corretamente.
...
\begin{exercise}
Defina uma cor nomeada (\var{green}) para o texto de um elemento usando a propriedade \var{color}.
\end{exercise}
...
\begin{exercise}
Defina uma cor de fundo para um elemento. A cor deve ser um tom de verde claro (\var{\#90EE90}).
\end{exercise}
...
\begin{exercise}
Defina uma cor de fundo para um elemento. A cor deve ser um tom de cinza escuro (\var{\#A9A9A9}).
\end{exercise}
